...
color:this.fontColor});for(var a=0;a<this.sharePanel.find("a").length;a++){var b=this.sharePanel.find("a")[a];$(b).find("span").css("color",this.mainColor)}},initHtml:function(){for(var a=0;a<this.sharePanel.find("a").length;a++){var b=this.sharePanel.find("a")[a],c=$("<span style='color:"+this.mainColor+"'>"+$(b).attr("title")+"</span>");$(b).append(c)}this.scrollStage=$("<div class='stage'></div>");this.swiper=$("<div class='swiper'></div>");this.progress=$("<div class='progress'></div>");this.progressBar=
$("<div class='progressBar'></div>");this.progress.append(this.progressBar);this.stage.append(this.title);this.favicon&&(bookConfig.RightToLeft?this.title.append(this.favicon):this.title.prepend(this.favicon));this.stage.append(this.close);this.getQrCodeVisible&&this.swiper.append(this.codeBox);this.swiper.append(this.sharePanel).append(this.linkTitle).append(this.linkArea);this.shareCurrentPageBox.append(this.shareCurrentCheck).append(this.shareCurrentPageTitle1);this.swiper.append(this.shareCurrentPageBox).append(this.copy1);
this.scrollStage.append(this.swiper).append(this.progress);this.stage.append(this.scrollStage);this._super();this.stage.css({height:windowHeight-20+"px"});this.scrollStage.css({height:windowHeight-63+"px"});this.scrollStage.scroll({},Direction.top)},changeItemColor:function(){this.elements.fontColor=this.fontColor;this.shareCurrentCheck.changeButtonColor(this.fontColor)},onShow:function(){var a=this;this.changeItemColor();this.close.changeButtonColor(this.fontColor);this.favicon&&this.favicon.changeButtonColor(this.fontColor);
this.scrollStage.refreshData();this.elements.addItem(function(){a.stage[0].style.display="inline-block"})}}).extend(SlideRightForm);var PcDownloadForm=Class({get hasFiles(){return window.downloadconfig?!downloadconfig.hideDownloadSinglePage||downloadconfig.pdf||downloadconfig.attachments&&0<downloadconfig.attachments.length:!1},create:function(a){this.elements=new DownloadElements(this);this._super(a)},onCreate:function(){this.initColorCss();this._super()},initColorCss:function(){this.stage.css({"background-color":this.mainColorObj.rgba(.95*this.formOpacity),color:this.fontColor});this.borderColor=this.mainColorObj.add(32);this.itemBgColor=
